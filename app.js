...
bot.on("messageCreate", async msg => {
  ...
  if(command.toLowerCase() === "channel") return channel()
  ...
  async function help() {
    return await msg.channel.createMessage({embed: {
      ...
      fields: [
        {
          name: 'FOR Endress Frontier Users. Free Tranlstation BOT **T-77**.',
          value: 'Thanks for TannerReynolds/Translation-Bot. MIT License.'
        },
        {
          name: 'Translating Messages Automatically',
          value: 'We allow automatic translations! To set up a channel to allow automatic translations between two languages, simply run **\":t channel (first language iso) (second language iso)\"**. This will translate all text to those the two selected languages! \n**Full list of language isos:** https://goo.gl/3Ftuqz'
        },
        {
          name: ':t channel none',
  ...
  async function channel(){
    ...
    if (!manageChannels) return await msg.channel.createMessage('Either you or the bot does not have the manageChannels permission')
